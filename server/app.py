...
class Scientists(Resource):
    def get(self):
        try:
            scientist_data = [scientist.to_dict(rules=("-missions",)) for scientist in Scientist.query.all()]
    ...
    def patch(self, id):
        ...
        try:
            for k in request.get_json():
                setattr(scientist_by_id, k, request.get_json()[k])
            db.session.add(scientist_by_id)
            db.session.commit()
            # db.session.refresh(scientist_by_id)
            return make_response(scientist_by_id.to_dict(rules=("-missions",)), 202)  
        except ValueError:
            db.session.rollback()
            return make_response({"errors" : ["validation errors"]}, 400)
...
class Planets(Resource):
    def get(self):
        planets = [planet.to_dict(rules=("-missions",)) for planet in Planet.query]
